...
class Waiter(coffeeHouse: ActorRef, barista: ActorRef, maxComplaintCount: Int)
    extends Actor
    with ActorLogging {
...
object Waiter {
  ...
  case class FrustratedException(coffee: Coffee, guest: ActorRef)
      extends IllegalStateException("Too main complaints!")

  def props(coffeeHouse: ActorRef,
            barista: ActorRef,
            maxComplaintCount: Int): Props =
